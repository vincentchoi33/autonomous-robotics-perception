...
class ImageProcessorNode(Node):
    def __init__(self):
        ...
        self.semantic_segmentation = SemanticSegmentation(num_classes=5)
    ...
    def process_and_publish(self):
        ...
        if self.segmentation_enabled and stitched_image is not None:
            try:
                start_time = time.time()
                
                # Perform segmentation
                segmentation_map, color_mask = self.semantic_segmentation.segment_image(stitched_image)
                
                # Create overlay (legend 없이)
                segmented_image = self.semantic_segmentation.overlay_segmentation(
                    stitched_image, segmentation_map, alpha=0.6
                )
                ...
                if self.frame_count % 30 == 0:
                    ...
                    self.get_logger().info(f'Segmentation stats - Avg time: {avg_time:.3f}s, FPS: {1/avg_time:.1f}')
                
                # Publish segmented image (legend 없이)
                ...
                # Fallback to stitched image only
                try:
                    ros_image = self.bridge.cv2_to_imgmsg(stitched_image, "bgr8")
                    self.processed_image_pub.publish(ros_image)
                except Exception as e2:
                    self.get_logger().error(f'Error publishing fallback image: {e2}')
        else:
            # Publish stitched image without segmentation
            try:
                ros_image = self.bridge.cv2_to_imgmsg(stitched_image, "bgr8")
                self.processed_image_pub.publish(ros_image)
            except Exception as e:
