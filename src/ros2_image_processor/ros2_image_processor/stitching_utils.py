...
_logger = rclpy.logging.get_logger('stitching_utils')
...
def load_homography_matrix(matrix_path):
    ...
    if matrix_path and os.path.exists(matrix_path):
        try:
            ...
            _logger.info(f'Homography matrix loaded from {matrix_path}')
            return H
        except Exception as e:
            _logger.error(f'Failed to load homography matrix: {e}')
            return None
    else:
        _logger.warn(f'Homography matrix file not found: {matrix_path}')
        return None
