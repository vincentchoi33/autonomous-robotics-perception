...
class SemanticSegmentation:
    def __init__(self, model_name="facebook/mask2former-swin-tiny-cityscapes-semantic", device=None):
        ...
        """
        self.logger = rclpy.logging.get_logger('seg')
        ...
        self.logger.info(f"Mask2Former({model_name}) loaded on {self.device}")
    ...
    def segment_image(self, image: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if len(unique_classes) > 0:
            self.logger.debug(f"Detected class indices: {unique_classes}")
            for class_id in unique_classes:
                if class_id < len(self.class_names):
                    self.logger.debug(f"   Class {class_id}: {self.class_names[class_id]}")
